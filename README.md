...
| 模型 | 特色 | 適合場景 |
|------|------|----------|
| **Kimi K2.5**（預設，建議） | 256K 超長上下文、速度快 | 長文件分析、日常對話 |
| GPT-4o | 多模態能力強 | 圖片分析、通用對話 |
| Claude（不建議） | 可能觸發供應商風控 | 不建議使用 |
| Gemini 3（不建議） | 可能觸發供應商風控 | 不建議使用 |

> 目前建議優先使用 Kimi。Kimi 官方入口：[http://kimi.com/code](http://kimi.com/code)
>  
> 風險提醒：使用 Claude / Gemini 可能導致帳號被封鎖，不建議使用。
...
- **只在需要時切換到其他模型** — 建議優先 GPT-4o；不建議使用 Claude / Gemini
...
### 想要新增功能或其他 AI 模型？
- 聯繫我們，我們可以幫你調整設定
- 可新增的項目包括：更多 AI 模型（需風險評估）、網路搜尋、語音模式、更多通訊平台（WhatsApp、Discord、LINE 等）
